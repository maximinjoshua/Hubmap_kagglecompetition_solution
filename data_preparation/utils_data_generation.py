...
def generate_data(filename, i, img_patch, mask_patch, config):
    # sz = img_patch.shape[0]
    # print(sz)
    # hsv = cv2.cvtColor(img_patch, cv2.COLOR_BGR2HSV)
    # h,s,v = cv2.split(hsv)
    # s_th = 80
    # p_th = 1000*(sz//256)**2
    # if (s>s_th).sum() <= p_th or img_patch.sum() <= p_th:
    #     return -1
    # else:
    if config['shift_h'] != 0:
        img_save_path = opj(config['OUTPUT_PATH'],filename+f'_img_{i:04d}_shifted512.jpg')
    else:
    ...
    if config['shift_h'] != 0:
        rle_save_path = opj(config['OUTPUT_PATH'],filename+f'_rle_{i:04d}_shifted512')
    else:
...
class HuBMAPDataset(Dataset):
    def __init__(self, df, filename, config, mode='train'):
        ...
        self.rle = df.loc[df['id']==int(filename), 'rle'].values[0]
    ...
    def __getitem__(self, idx): # idx = i_h * self.num_w + i_w
        ...
        if self.shift_h != 0:
            if i_h == 0 or i_w == 0:
                return {'img':np.array([-1]), 'mask':np.array([-1])}
        ...
        if self.reduce != 1:
            img_patch = cv2.resize(img_patch,(self.sz//self.reduce,self.sz//self.reduce),
                             interpolation = cv2.INTER_AREA)
            mask_patch = cv2.resize(mask_patch,(self.sz//self.reduce,self.sz//self.reduce),
