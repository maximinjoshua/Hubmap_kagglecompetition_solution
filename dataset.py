...
class HuBMAPDatasetTrain(Dataset):
    def __init__(self, df, config, mode='train'):
        ...
        self.h, self.w = self.config['input_resolution'], self.config['input_resolution']
    ...
    def __getitem__(self,idx):
        ...
        with open(rle_path,'rb') as f:
            rle = pickle.load(f)
        mask = rle2mask(rle, shape=(self.config['resolution'], self.config['resolution']))
